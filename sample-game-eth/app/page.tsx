...
import {
  ...
  ChainButton,
  useEthWallet,
} from "@/components/eth/eth-provider";
import {
  CLIENT_SLOT_ITEMS,
  type ClientSlotItem,
  type Rarity,
...
export default function Home() {
  const { address, walletClient, publicClient, targetChainId, switchChain } = useEthWallet();
  ...
  const shuffleItem = async () => {
    ...
    try {
      ...
      // Always request switch to target chain before submitting tx.
      // This avoids stale chain state when wallet starts on a different network.
      await switchChain(targetChainId);
      const { finalItem } = await runShuffleFlow(
        walletClient,
        publicClient,
        address
      );
      clearInterval(shuffleInterval);
      commitResult(finalItem);
    } catch (err) {
      clearInterval(shuffleInterval);
