...
import { getClientEnv, type Cluster } from "@/config/env";
...
export const useCluster = () => {
  ...
  if (!context) {
    throw new Error("useCluster must be used within UmiProvider");
  }
  return context;
};
...
export function UmiProvider({ children }: { children: ReactNode }) {
  const { defaultCluster } = getClientEnv();
  const [cluster, setCluster] = useState<Cluster>(defaultCluster);

  const endpoint = useMemo(() => {
    if (cluster === "localnet") return LOCALNET_RPC;
