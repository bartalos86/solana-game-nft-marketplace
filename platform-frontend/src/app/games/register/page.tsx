...
function encodeSecretKeyBase64(secretKey: Uint8Array): string {
  if (typeof Buffer !== 'undefined') {
    return Buffer.from(secretKey).toString('base64')
  }
  return btoa(String.fromCharCode(...secretKey))
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const regResult = await registerGameOnChainAction({
        ...
        authorityPublicKey: parsed.data.solanaPublicKey,
      })
      if (regResult.error) {
      ...
      router.push(`/games/${keys.solanaPublicKey}`)
    } catch (err) {
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <form onSubmit={(e) => handleSubmit(e as React.FormEvent)} className="space-y-5">
          ...
          <div className="rounded-2xl border border-blue-500/20 bg-blue-950/20 p-6 backdrop-blur-xl">
            ...
            {/* Solana keys */}
            <div className="mb-3 rounded-xl border border-white/7 bg-black/10 p-4">
              <div className="mb-3 flex items-center gap-2">
                <div className="h-5 w-5 rounded-full bg-linear-to-br from-purple-400 to-indigo-500 p-0.5">
                  <div className="h-full w-full rounded-full bg-black/30" />
                </div>
                <span className="text-xs font-semibold text-white/60 uppercase tracking-wider">Solana</span>
              </div>
              <div className="space-y-3">
                <KeyRow label="Public key" value={keys.solanaPublicKey} isPrivate={false} />
                <KeyRow label="Secret key (base64)" value={keys.solanaPrivateKey} isPrivate />
              </div>
            </div>
