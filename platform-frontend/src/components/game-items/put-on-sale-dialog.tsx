...
interface PutOnSaleDialogProps {
  ...
  itemName: string
  onConfirm: (priceSol: string, expiryDays: number) => Promise<void>
}
...
export function PutOnSaleDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    const num = parseFloat(priceSol)
    if (Number.isNaN(num) || num < MIN_PRICE) {
      setError(`Minimum price is ${MIN_PRICE} SOL`)
      return
    }
    ...
    setLoading(true)
    try {
      await onConfirm(priceSol, expiryDays)
      onOpenChange(false)
      setPriceSol('')
